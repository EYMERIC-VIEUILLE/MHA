...
public class TestSign
{
	...
	{
		try {
			BigInteger modulo = new BigInteger(1, Base64.decode(
					"APFbzo28aj0laYgu6jpt7MVHGRYi/URzT+h+L2poQSTIGg/tLZU4/Ib2a3NvPh/Oe4AjsHf7TAdQ7dFtOzX4Uvvbyr2kdi2GD0zSYxtDulhhBgQ9coUWZymXKyvx8zK2DIes+j7oHZM2znzu64fj3DYv2Kut98t6CgtqZMrXhNCb"));
			...
			System.err.println(
					"Avertissement : vous ne disposez pas de bibliothèque permettant de vérifier l'authenticité des profils");
			return false;
		}
	}
	...
	{
		try {
			if (verifSignature("Ceci est un message à signer ! Si si",
	...
	public static void genereCle()
	{
		try {
			...
			System.out.println(
					"Modulo : " + Base64.encodeBytes(((RSAPublicKey) kp.getPublic()).getModulus().toByteArray()));
			System.out.println("Exposant Public : "
					+ Base64.encodeBytes(((RSAPublicKey) kp.getPublic()).getPublicExponent().toByteArray()));
			System.out.println("Exposant Privé : "
					+ Base64.encodeBytes(((RSAPrivateKey) kp.getPrivate()).getPrivateExponent().toByteArray()));
		} catch (NoSuchAlgorithmException e) {
			System.err.println(
					"Avertissement : vous ne disposez pas de bibliothèque permettant de vérifier l'authenticité des profils");
		}
