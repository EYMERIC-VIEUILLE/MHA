...
public class MHAGame implements Serializable {
	...
	private String lowLevelAttaque(String nomA,Troll a,Troll d,int dA,int bmA,int dE,int bmE, int seuil, int dD,int dDC,int bmD, int dArN, int armure,int seuil_decamoufle,boolean peutEtreContreAttaque)
	{
		...
		if(peutEtreContreAttaque && !d.isDead() && d.getNbCA()>0)
		{
			...
			s2+="\nVous aviez programmé une contre-attaque\n"+lowLevelAttaque(" avec une compétence", d , a , (d.getAttaque())/2,  d.getBMAttaque()/2, a.getEsquive() , a.getBMEsquive(), 0 , d.getDegat(),(3*d.getDegat())/2,d.getBMDegat(),a.getArN(), a.getArmurePhy()+a.getArmureMag(),100,false);
	...
	public String attaquePrecise(Troll t)
	{
		...
		if(((Integer) lo[0]) > 0)
			return s + "\n" + lowLevelAttaque(" avec une compétence", currentTroll , t , currentTroll.getAttaque() + Math.min(((Integer) lo[0])*3, currentTroll.getAttaque()/2),  currentTroll.getBMAttaque(), t.getEsquive() , t.getBMEsquive(), 0 , currentTroll.getDegat(),(3*currentTroll.getDegat())/2,currentTroll.getBMDegat(),t.getArN(),t.getArmurePhy()+t.getArmureMag(),100,true);
		else return s;
	}
	...
	public String botteSecrete(Troll t)
	{
		...
		String s= lo[1].toString();
		if(((Integer) lo[0]) > 0)
			return s + "\n" + lowLevelAttaque(" avec une compétence", currentTroll , t , currentTroll.getAttaque()/2,  currentTroll.getBMAttaque()/2, t.getEsquive() , t.getBMEsquive(), 0 , currentTroll.getAttaque()/2,(3*(currentTroll.getAttaque()/2))/2,currentTroll.getBMDegat()/2,t.getArN(), (t.getArmurePhy()+t.getArmureMag())/2,100,true);
		else return s;
	}
	...
	public String charger(Troll t)
	{
		...
		if(((Integer) lo[0]) > 0)
		{
			...
			s += "\n" + lowLevelAttaque(" avec une compétence", currentTroll , t , currentTroll.getAttaque(),  currentTroll.getBMAttaque(), t.getEsquive() , t.getBMEsquive(), 0 , currentTroll.getDegat(),(3*currentTroll.getDegat())/2,currentTroll.getBMDegat(),t.getArN(), t.getArmurePhy()+t.getArmureMag(),100,true);
	...
	public String explosion()
	{
		...
		if(s.indexOf("RÉUSSI")!=-1)
		{
			...
			for(int i=0;i<trolls.size();i++)
			{
				...
				{
					s += "\n" + lowLevelAttaque(" avec un sortilège", currentTroll , t , -1,  0, 0 , 0, calculeSeuil(currentTroll.getMM(),t.getRM()) , deg,0,0,0,0,100,false);
					b=true;
				}
			}
			if(!b)
	...
	public String rafalePsychique(Troll t)
	{
		...
		if(s.indexOf("RÉUSSI")!=-1)
		{
			s += "\n" + lowLevelAttaque(" avec un sortilège", currentTroll , t , -1,  0, 0 , 0, calculeSeuil(currentTroll.getMM(),t.getRM()) , currentTroll.getDegat(),(3*currentTroll.getDegat())/2,currentTroll.getBMMDegat(),0,t.getArmureMag(),100,false);
			BM bm;
			if(s.indexOf("EFFET REDUIT")==-1)
	...
	public String vampirisme(Troll t)
	{
		...
		{
			int pv=t.getPV();
			s += "\n" + lowLevelAttaque(" avec un sortilège ", currentTroll , t , (2*currentTroll.getDegat())/3,  currentTroll.getBMMAttaque(), t.getEsquive() , t.getBMEsquive()+t.getBMMEsquive(), calculeSeuil(currentTroll.getMM(),t.getRM()) , currentTroll.getDegat(),(3*currentTroll.getDegat())/2,currentTroll.getBMMDegat(),0,t.getArmureMag(),100,true);
	...
	private String appliquePiege(Piege p)
	{
		...
		for(int i=0;i<trolls.size();i++)
		{
			...
			if(t.getPosX()==p.getPosX() && t.getPosY()==p.getPosY() && t.getPosN()==p.getPosN() && !t.isDead())
			{
				...
				s1+="\n"+lowLevelAttaque(" avec une compétence",p.getCreateur(),t,-1,0,0,0,seuil,p.getDegat(),0,0,0,0,0,false);
				if(t==currentTroll)
				{
	...
	public String prendreTP()
	{
		...
		BM bm = new BM("Désorientation",-diceHelper.roll(dist/10,6),-diceHelper.roll(dist/10,6),0,0,0,0,0,0,0,0,0,false,1);
