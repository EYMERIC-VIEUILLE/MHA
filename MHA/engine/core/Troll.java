...
public class Troll {
	...
	public Troll(String n, int i,int s, int c,int r) {
		...
		duree_tour=12*60;
		pa=0;
		debut_tour = diceHelper.roll(1, duree_tour) - 1;
		date_jeu=debut_tour;
		if(r==RACE_SKRIM)
		{
			race=r;
	...
	public void setProfil(int niv,int att,int esq,int deg,int reg, int arN, int pv, int v,int dla,int resm, int maim)
	{
		niveau=niv;
		des_Attaque=att;
		des_Esquive=esq;
	...
	public String beginTurn(int current)
	{
		...
		if(isDead() && nbResu>0)
		{
			...
			if(MHAGame.getMode()==MHAGame.MODE_TEAM_DEATHMATCH && MHAGame.isRegroupe())
			{
				...
				setPos(diceHelper.roll(1, MHAGame.getSizeArena()) - 1, diceHelper.roll(1, MHAGame.getSizeArena()) - 1,
						-diceHelper.roll(1, (MHAGame.getSizeArena() + 1) / 2));
		...
		int i = Math.max(diceHelper.roll(des_Regeneration, 3) + bmRegeneration + bmmRegeneration, 0);
		int j=i-venin;
		...
		if(fatigue>0)
		{
			fatigue=(int) Math.floor( (fatigue)/1.25);
